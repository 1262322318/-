...
-- 【状态：正式  合入日期：2026-06-18  草稿来源：drafts/dws_ipd_ipm_qihua_hit_detail_dd_zhengshiban_draft.sql】
-- ******************************************************************** --
...
--   2  = 未停产 + 上市7-18月
--   3  = 未停产 + 上市19-23月
--   41 = 未停产 + 上市=24月 + 未停止下单（考核：企划命中率-在产）
...
,stage_calc AS (
    SELECT pb.*
        ...
        ,CASE
            ...
            WHEN pb.stop_production_date IS NULL AND pb.shangshi_month = 24 AND pb.stop_order_date IS NULL THEN 41  -- 仅=24月在产
            -- 阶段7：已停止下单 + 未停产
...
SELECT
    ...
    ,CASE
        ...
        WHEN sc.stage IN (2,3,41,42) THEN sc.max_rolling_12m_qty / NULLIF(sc.plan_first_year_qty, 0)
    ...
    -- 阶段（直接输出，无歧义）
    ,sc.stage            AS stage                -- 阶段编号
    -- 阶段标签
    ,CASE sc.stage
        WHEN 1  THEN '上市1-6个月'
        WHEN 2  THEN '上市7-18个月'
        WHEN 3  THEN '上市19-23个月'
        WHEN 41 THEN '上市24个月(企划命中率-在产)'
    ...
    ,CASE
        ...
        WHEN sc.stage IN (3,41,42) THEN CASE WHEN sc.max_rolling_12m_qty < sc.plan_first_year_qty THEN 'N' ELSE 'Y' END
    ...
    ,CASE WHEN sc.stage IN (1,2,3,41,42,43) THEN 'Y' ELSE 'N'
    END                  AS is_in_hongheibang    -- 是否上红黑榜
    ,CASE WHEN sc.stage IN (41,42,43) THEN 'Y' ELSE 'N'
