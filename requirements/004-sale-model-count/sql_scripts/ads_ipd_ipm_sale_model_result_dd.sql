...
-- ******************************************************************** --
-- author: lvxuebin.ex
-- create time: 2026/01/13 14:18:29 GMT+08:00
-- ******************************************************************** --
delete from ads.ads_ipd_ipm_sale_model_result_dd
where company in ('冰冷','洗衣机','空调公司','视像科技','厨电','集团汇总','激光')  --同时插入集团汇总计划值
and dt_month >= DATE_FORMAT('${GP_START_DT}', '%Y%m') 
and data_type = '在销型号-产品型号口径'
;

insert into ads.ads_ipd_ipm_sale_model_result_dd(
dt_month	--月份
,dt_type	--日期类型
,data_type	--数据类型
...
,t2.dt_month
,t3.dt_type
from weidu_shiyebu t1 ,dt_month_weidu t2 ,weidu_dt_type t3
)
,weidu_neiwaixiao as (
...
,count(distinct t1.model) act_value
from dws.dws_ipd_ipm_sale_model_detail_dd t1 
...
,case when t2.in_out_sale = '全部' then '全部' else t1.in_out_sale end,t1.dt_type
,t1.business_division
)
select 
t0.dt_month
...
,t1.act_value
,t2.plan_zxmodel
,2-(nullif(coalesce (t1.act_value,0.0),0.0)/nullif(coalesce (t2.plan_zxmodel,0.0),0.0))  完成率
,now()
from all_weidu t0
...
,t1.act_num	--实际值
,t2.plan_zxmodel --计划值
,2-(coalesce (t1.act_num,0.0)/nullif(coalesce (t2.plan_zxmodel,0.0),0.0))  完成率
,now()
from act_value t1 
left join plan_values t2 
...
delete from ads.ads_ipd_ipm_sale_model_result_dd
where product_line in ('冰箱','冷柜','洗衣机','视像科技') 
and dt_month = DATE_FORMAT('${GP_START_DT}', '%Y%m')
and data_type in ('在销型号-销售型号口径','在销型号-销售型号编码口径')
and dt_type = '月'
;
...
,'在销型号-销售型号口径' as data_type
,count(distinct salemodel) 
,now()
from dws.dws_ipd_ipm_salemodelid_dd t1 
left join weidu_neiwaixiao t2 on 1=1
where dt_month = DATE_FORMAT('${GP_START_DT}', '%Y%m') 
and product_line in ('冰箱','冷柜','洗衣机','视像科技') 
and dt_type ='月'
and is_project = 'N'
and in_out_sale is not null 
group by dt_month 
,dt_type 
,company 
,product_line 
...
,product_line 
,case when t2.weidu_neiwaixiao = '全部' then '全部' else in_out_sale end as in_out_sale
,'在销型号-销售型号编码口径' as data_type
,count(distinct salemodelid) 
,now()
from dws.dws_ipd_ipm_salemodelid_dd t1 
left join weidu_neiwaixiao t2 on 1=1
where dt_month = DATE_FORMAT('${GP_START_DT}', '%Y%m') 
