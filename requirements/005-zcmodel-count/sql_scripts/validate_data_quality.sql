-- DORIS sql
-- ******************************************************************** --
-- 脚本名称: validate_data_quality.sql
-- 功能描述: 在产型号数 - 数据质量检查
--           在ETL脚本执行完成后运行，验证数据完整性和准确性
-- 作者: ETL智能辅助工具
-- 创建时间: 2026-05-09
-- 使用方式: 每次ETL执行后运行，检查结果应全部为PASS
-- ******************************************************************** --


-- ====================================================================
-- 1. DWS层：在产型号明细表 行数检查
-- 预期：当月数据行数 > 0
-- ====================================================================
SELECT
    'dws_zcmodel_detail_dd' AS check_table
    ,'行数检查' AS check_type
    ,COUNT(*) AS row_count
    ,CASE WHEN COUNT(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result
FROM dws.dws_ipd_ipm_zcmodel_detail_dd
WHERE dt_month = DATE_FORMAT(date_sub('${GP_START_DT}', INTERVAL 1 MONTH), '%Y%m');


-- ====================================================================
-- 2. DWS层：关键字段空值率检查
-- 预期：model、product_line 空值率 = 0
-- ====================================================================
SELECT
    'dws_zcmodel_detail_dd' AS check_table
    ,'关键字段空值率' AS check_type
    ,COUNT(*) AS total_rows
    ,SUM(CASE WHEN model IS NULL OR model = '' THEN 1 ELSE 0 END) AS null_model
    ,SUM(CASE WHEN product_line IS NULL OR product_line = '' THEN 1 ELSE 0 END) AS null_product_line
    ,CASE
        WHEN SUM(CASE WHEN model IS NULL OR model = '' THEN 1 ELSE 0 END) = 0
         AND SUM(CASE WHEN product_line IS NULL OR product_line = '' THEN 1 ELSE 0 END) = 0
        THEN 'PASS' ELSE 'FAIL'
    END AS result
FROM dws.dws_ipd_ipm_zcmodel_detail_dd
WHERE dt_month = DATE_FORMAT(date_sub('${GP_START_DT}', INTERVAL 1 MONTH), '%Y%m');


-- ====================================================================
-- 3. DWS层：在产型号数应小于等于在销型号数
-- 预期：在产型号数 <= 在销型号数（同产品线同月份）
-- ====================================================================
SELECT
    'dws_zcmodel_detail_dd' AS check_table
    ,'在产<=在销校验' AS check_type
    ,zc.product_line
    ,zc.zc_count AS zcmodel_count
    ,sm.sm_count AS sale_model_count
    ,CASE WHEN zc.zc_count <= sm.sm_count THEN 'PASS' ELSE 'FAIL' END AS result
FROM (
    SELECT product_line, COUNT(DISTINCT model) AS zc_count
    FROM dws.dws_ipd_ipm_zcmodel_detail_dd
    WHERE dt_month = DATE_FORMAT(date_sub('${GP_START_DT}', INTERVAL 1 MONTH), '%Y%m')
      AND is_project = 'N'
    GROUP BY product_line
) zc
LEFT JOIN (
    SELECT product_line, COUNT(DISTINCT model) AS sm_count
    FROM dws.dws_ipd_ipm_sale_model_detail_dd
    WHERE dt_month = DATE_FORMAT(date_sub('${GP_START_DT}', INTERVAL 1 MONTH), '%Y%m')
      AND is_project = 'N'
    GROUP BY product_line
) sm ON zc.product_line = sm.product_line;


-- ====================================================================
-- 4. ADS层：在产型号数结果表 行数检查
-- 预期：当月数据行数 > 0
-- ====================================================================
SELECT
    'ads_zcmodel_result_dd' AS check_table
    ,'行数检查' AS check_type
    ,COUNT(*) AS row_count
    ,CASE WHEN COUNT(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result
FROM ads.ads_ipd_ipm_zcmodel_result_dd
WHERE dt_month = DATE_FORMAT(date_sub('${GP_START_DT}', INTERVAL 1 MONTH), '%Y%m');


-- ====================================================================
-- 5. ADS层：在产型号数合理性检查
-- 预期：act_num > 0
-- ====================================================================
SELECT
    'ads_zcmodel_result_dd' AS check_table
    ,'型号数合理性' AS check_type
    ,COUNT(*) AS total_rows
    ,SUM(CASE WHEN act_num <= 0 THEN 1 ELSE 0 END) AS zero_or_negative
    ,CASE
        WHEN SUM(CASE WHEN act_num <= 0 THEN 1 ELSE 0 END) = 0
        THEN 'PASS' ELSE 'FAIL'
    END AS result
FROM ads.ads_ipd_ipm_zcmodel_result_dd
WHERE dt_month = DATE_FORMAT(date_sub('${GP_START_DT}', INTERVAL 1 MONTH), '%Y%m');
