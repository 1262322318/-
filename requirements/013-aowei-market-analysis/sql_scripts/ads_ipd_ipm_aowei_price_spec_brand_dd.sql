/*
 * 脚本名称: ads_aowei_price_spec_brand.sql
 * 功能描述: 表2-5 分价格段分规格段分品牌数据（指标行格式）
 * 作者: Kiro Agent
 * 创建时间: 2026-07-08
 * 修改记录:
 ...
 *   输出: ads.ads_ipd_ipm_aowei_price_spec_brand_dd
 * 业务规则:
 *   1. 只展示筛选品牌（从表2-1聚合）
 *   2. 占有率分母：同品类细分+同渠道+同价格段+同规格段下的全市场总量
 *   3. 转为指标行格式
 */

truncate table ads.ads_ipd_ipm_aowei_price_spec_brand_dd;

INSERT INTO ads.ads_ipd_ipm_aowei_price_spec_brand_dd (
    business_unit,       -- 事业部
...
brand_sales AS (
    SELECT
        ...
        MAX(CASE WHEN metric_name = '所属规格段' THEN val_curr END) AS spec_segment
    FROM ads.ads_ipd_ipm_aowei_model_price_spec_dd
    WHERE metric_name IN ('销额','销量','所属价格段','所属规格段')
...
brand_ps_agg AS (
    ...
    FROM brand_sales
    WHERE price_segment IS NOT NULL AND spec_segment IS NOT NULL
    GROUP BY business_unit, prdct_line_name, product_mid_class, product_small_class,
             category_segment, o2o_type, price_segment, spec_segment, stat_brand
),

-- 占有率分母：从表2-4取同品类细分+同渠道+同价格段+同规格段的全市场总量
market_total_amt AS (
    SELECT
        business_unit, prdct_line_name, product_mid_class, product_small_class,
...
with_share AS (
    SELECT
        bpa.*,
        mta.total_amt_y3, mta.total_amt_y2, mta.total_amt_y1,
        mta.total_amt_curr, mta.total_amt_y1_ytd,
        mtq.total_qty_y3, mtq.total_qty_y2, mtq.total_qty_y1,
        mtq.total_qty_curr, mtq.total_qty_y1_ytd
    FROM brand_ps_agg bpa
    LEFT JOIN market_total_amt mta
        ON bpa.business_unit = mta.business_unit AND bpa.prdct_line_name = mta.prdct_line_name
        AND bpa.product_mid_class = mta.product_mid_class AND bpa.product_small_class = mta.product_small_class
        AND bpa.category_segment = mta.category_segment AND bpa.o2o_type = mta.o2o_type
        AND bpa.price_segment = mta.price_segment AND bpa.spec_segment = mta.spec_segment
    LEFT JOIN market_total_qty mtq
        ON bpa.business_unit = mtq.business_unit AND bpa.prdct_line_name = mtq.prdct_line_name
        AND bpa.product_mid_class = mtq.product_mid_class AND bpa.product_small_class = mtq.product_small_class
        AND bpa.category_segment = mtq.category_segment AND bpa.o2o_type = mtq.o2o_type
        AND bpa.price_segment = mtq.price_segment AND bpa.spec_segment = mtq.spec_segment
)
...
SELECT business_unit, prdct_line_name, product_mid_class, product_small_class,
       category_segment, o2o_type, price_segment, spec_segment, stat_brand, '总销量',
...
-- 指标3：产品均价
SELECT business_unit, prdct_line_name, product_mid_class, product_small_class,
       category_segment, o2o_type, price_segment, spec_segment, stat_brand, '产品均价',
...
-- 指标4：额占有率
SELECT business_unit, prdct_line_name, product_mid_class, product_small_class,
       category_segment, o2o_type, price_segment, spec_segment, stat_brand, '额占有率',
...
-- 指标5：量占有率
SELECT business_unit, prdct_line_name, product_mid_class, product_small_class,
       category_segment, o2o_type, price_segment, spec_segment, stat_brand, '量占有率',
